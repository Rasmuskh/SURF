...
def setMicroStepMode(n):
    ...
    comm.write(addr+'N'+n++'R'+'\r')#This command is not run untill an R has been received
    res = comm.read()
    return res
def setBacklashIncrements(n):
    ...
    comm.write(addr+'K'+n++'R'+'\r')#This command is not run untill an R has been received
    res = comm.read()
    return res
def setPumpConfig(n):
